...
class EnterAddressBottomSheet extends StatefulWidget {
  ...
  @override
  State<EnterAddressBottomSheet> createState() => _EnterAddressBottomSheetState();
}
...
class _EnterAddressBottomSheetState extends State<EnterAddressBottomSheet> {
  ...
  void _confirmAddress() {
    ...
    final newAddress = {
      ...
      'icon': _selectedAddressType == 'Home' 
          ? Icons.home_outlined 
          : (_selectedAddressType == 'Work' ? Icons.work_outline : Icons.location_on_outlined),
      'distance': '0 m',
      'address': '${_houseController.text}, ${_floorController.text.isNotEmpty ? 'Floor ${_floorController.text}, ' : ''}${widget.address}',
      'phone': userPhone,
                ...
                child: RichText(
                  text: TextSpan(
                    ...
                    children: [
                      ...
                      TextSpan(text: userPhone, style: const TextStyle(fontWeight: FontWeight.bold)),
                    ],
                  ),
                ),
              ),
          ...
          SingleChildScrollView(
            ...
            child: Row(
              children: [
                ...
                const SizedBox(width: 8),
                _addressChip(label: "Hotel", icon: Icons.apartment_outlined),
                const SizedBox(width: 8),
                _addressChip(label: "", icon: Icons.location_on_outlined),
              ],
            ),
          ),
            ...
            child: Row(
              ...
              children: [
                Expanded(
                  child: Text(
                    widget.address,
                    style: GoogleFonts.poppins(color: const Color(0xFF757575), fontSize: 13),
                  ),
                ),
                const SizedBox(width: 8),
                OutlinedButton(
    ...
    return GestureDetector(
      ...
      child: Container(
        ...
        decoration: BoxDecoration(
          ...
          borderRadius: BorderRadius.circular(10),
          border: Border.all(color: isSelected ? const Color(0xFFC62828) : const Color(0xFF3A3A3A)),
        ),
        child: Row(
  ...
  Widget _buildInputFields() {
    return Padding(
      ...
      child: Column(
        children: [
          _buildInputField(hint: "House number *", controller: _houseController),
          _buildInputField(hint: "Floor", controller: _floorController),
          _buildInputField(hint: "Tower / Block (optional)", controller: _towerController),
        ],
      ),
    );
  }

  Widget _buildInputField({required String hint, required TextEditingController controller}) {
    return Container(
      ...
      child: TextField(
        ...
        decoration: InputDecoration(
          hintText: hint,
          hintStyle: GoogleFonts.poppins(color: const Color(0xFF757575), fontSize: 14),
          filled: true,
      ...
      child: AnimatedContainer(
        ...
        child: ElevatedButton(
          ...
          style: ElevatedButton.styleFrom(
            backgroundColor: _isFormValid ? const Color(0xFFE53935) : const Color(0xFF4A4A4A),
