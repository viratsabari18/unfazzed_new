...
Future<void> _loadCustomIcons() async {
  ...
  Future<void> _fetchRiderLocation(String bookingId) async {
    try {
      ...
      final url = Uri.parse('${ApiConfig.apiBaseUrl}/get-location?booking_id=$bookingId');
        ...
        if (mounted) {
          setState(() {
            ...
            if (_currentStep > 0 && _currentStep < _routePoints.length * 0.9) {
              _simulatedState = BookingState.onTheWay; 
            } else if (_currentStep >= _routePoints.length * 0.9 && _currentStep < _routePoints.length - 1) {
              _simulatedState = BookingState.started; 
      ...
      if (response.statusCode == 200) {
        ...
        if (mounted) {
          setState(() {
            ...
            if (currentStatus == 'accept') _backendStatus = "Accepted";
            else if (currentStatus == 'ongoing' || currentStatus == 'on_going') _backendStatus = "On the Way";
            else if (currentStatus == 'arrived') _backendStatus = "Arrived";
            else _backendStatus = currentStatus;
          });
        }
        ...
        if (currentStatus == 'arrived' || 
           ...
           debugPrint("MATCH FOUND! Redirecting to progress/verification screen...");
...
''';
  ...
  Widget build(BuildContext context) {
    ...
    final _bData = _currentBookingData is List ? (_currentBookingData as List).first : _currentBookingData;
    final _rawProv = _bData?['provider_data'];
    final _provider = _rawProv is List ? (_rawProv.isNotEmpty ? _rawProv.first : {}) : (_rawProv ?? {});
    final _rawSvc = _bData?['service'];
    final _service = _rawSvc is List ? (_rawSvc.isNotEmpty ? _rawSvc.first : {}) : (_rawSvc ?? {});
    final _rawDet = _bData?['booking_detail'];
    final _detail = _rawDet is List ? (_rawDet.isNotEmpty ? _rawDet.first : {}) : (_rawDet ?? {});
    final _rawHandy = _bData?['handyman_data'];
    final _handy = _rawHandy is List ? (_rawHandy.isNotEmpty ? _rawHandy.first : {}) : (_rawHandy ?? {});

    final pro = ProfessionalMatch(
      name: _handy['display_name']?.toString() ?? _provider['display_name']?.toString() ?? _detail['provider_name']?.toString() ?? "Professional",
      rating: (_handy['handyman_rating'] ?? _provider['providers_service_rating'] ?? _provider['handyman_rating'] ?? 0).toDouble(),
      jobsDone: (_handy['total_services_booked'] ?? _provider['total_services_booked'] ?? 0) as int,
      avatarUrl: _handy['profile_image']?.toString() ?? _service['provider_image']?.toString() ?? _provider['profile_image']?.toString() ?? "lib/assets/images/rider_image.png",
    );
...
polylines: _routePoints.isEmpty
      ...
      children: [
        CircleAvatar(
          ...
          backgroundImage: (profileImageUrl != null && profileImageUrl.startsWith('http'))
              ? CachedNetworkImageProvider(profileImageUrl, headers: const {}) as ImageProvider
        ...
        Expanded(
          child: Column(
            ...
            children: [
              ...
              const SizedBox(height: 4),
              Text(
                _backendStatus == "Arrived" ? "Professional is at your door" : "On their way to your location",
                style: TextStyle(
                  color: _backendStatus == "Arrived" ? const Color(0xFF4CAF50) : const Color(0xFF6366F1),
        ...
        Expanded(
          child: GestureDetector(
            onTap: () {
              ...
              bool hasHandyman = handyman != null && (handyman['id'] != null || handyman['uid'] != null);
              bool hasProvider = provider != null && (provider['id'] != null || provider['uid'] != null);

              void navigateToChat(dynamic target, String label) {
                Navigator.pushNamed(
                  context,
                  AppRoutes.chatHomeScreen,
                  arguments: {
                    'name': target?['display_name'] ?? detail['provider_name'] ?? label,
                    'image': target?['profile_image'] ?? service?['provider_image'] ?? target?['profile_image'] ?? "lib/assets/images/rider_image.png",
                    'phone': target?['contact_number']?.toString(),
                    'booking_id': detail?['id']?.toString(),
                    'provider_uid': target?['uid']?.toString(),
                    'handyman_uid': target?['uid']?.toString(),
                    'handyman_id': target?['uid']?.toString() ?? target?['id']?.toString(),
                  },
                );
              }

              navigateToChat(handyman?['id'] != null ? handyman : provider, "Professional");
            },
            child: _buildActionButton(
  ...
  Widget _buildServiceDetails() {
    ...
    if (widget.bookingData != null) {
      ...
      image = (widget.service as ServiceData).providerImage ?? UserMessages.serviceBookingDummy1;
          ...
          children: [
            ...
            ClipRRect(
              ...
              child: isNetworkImage
                  ...
                  : Image.asset(image, width: 100, height: 70, fit: BoxFit.cover),
            ),
          ],
        ),
      ],
      ...
      children: [
        Expanded(
          child: GestureDetector(
            ...
            child: _buildFooterButton(Icons.support_agent, "Support", Colors.black),
          ),
        ),
      ],
    );
