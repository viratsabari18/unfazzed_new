...
class _PaymentsHomePageState extends State<PaymentsHomePage> {
  ...
  Future<void> _fetchGateways() async {
    ...
    final gateways = await _paymentService.fetchPaymentGateways(token: userProvider.apiToken);
    ...
    if (mounted) {
      setState(() {
        ...
        if (_gateways.isNotEmpty) {
          _selectedGateway = _gateways.firstWhere((g) => g['type'] == 'cash', orElse: () => _gateways.first);
  ...
  Widget build(BuildContext context) {
    ...
    // Data extraction
    final serviceName = detail?['service_name'] ?? service?['name'] ?? UserMessages.fullHomeCleaning;
    ...
    final double discountPercent = (detail?['discount'] ?? service?['discount'] ?? 0).toDouble();
              ...
              child: SingleChildScrollView(
                ...
                child: Column(
                  ...
                  children: [
                    ...
                    billRow(UserMessages.serviceFee, "₹ $serviceFee", AppColors.naturalBlack),
                    ...
                    if (discountAmount > 0)
                      billRow(UserMessages.coupon, "- ₹${discountAmount.toStringAsFixed(0)}", AppColors.softBlue),
                    ...
                    if (_isLoading)
                      ...
                      ..._gateways.map((g) => paymentTile(
                        title: g['title'] ?? "Payment Method",
                        value: g['type'] ?? "",
                        gateway: g,
                      )).toList(),
  ...
  void _processRazorPay() {
    ...
    var options = {
      'key': key,
      'amount': (totalAmount * 100).toInt(), // amount in the smallest currency unit
  ...
  void _handleRazorPaySuccess(PaymentSuccessResponse response) async {
    ...
    if (success) {
      _showSuccessDialog();
       ...
       _showErrorDialog("Payment successful but failed to update status on server. Please contact support.");
  ...
  void _processWalletPayment() async {
    ...
    if (success) {
      ...
      _showErrorDialog("Insufficient wallet balance or failed to process wallet payment.");
    }
  }
        ...
        child: Container(
          ...
          child: Column(
            ...
            children: [
              ...
              const Text(
                "Your service booking is now confirmed. Our professional will arrive at the scheduled time.",
            ...
            children: [
              Container(
                ...
                decoration: BoxDecoration(
                  ...
                  shape: BoxShape.circle,
                  border: Border.all(color: AppColors.primaryRed.withOpacity(0.2), width: 4),
                ),
                child: const Icon(Icons.error_outline_rounded, color: AppColors.primaryRed, size: 48),
              ),
              const SizedBox(height: 24),
              const Text(
                "Payment Failed",
          ...
          child: Column(
            ...
            children: [
              Container(
                ...
                child: const Icon(Icons.handshake_rounded, color: Colors.blue, size: 48),
              ),
              const SizedBox(height: 24),
              const Text(
                "Booking Confirmed!",
