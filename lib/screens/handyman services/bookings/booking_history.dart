...
class _BookingHistoryState extends State<BookingHistory> {
  ...
  Future<void> _fetchBookings() async {
    ...
    final response = await _bookingService.fetchBookingList(token: userProvider.apiToken);
            ...
            : RefreshIndicator(
                ...
                child: ListView.builder(
                  ...
                  itemBuilder: (_, i) {
                    ...
                    final statusLabel = item['status_label']?.toString() ?? status.value;
                    
                    final rawHandyman = item['handyman_data'];
                    final handyman = rawHandyman is List ? (rawHandyman.isNotEmpty ? rawHandyman.first : null) : rawHandyman;
                    ...
                    final imageUrl = (handyman?['profile_image'] != null)
                        ...
                        : (attachments != null && attachments.isNotEmpty) 
                            ...
                            : item['provider_image']?.toString() ?? UserMessages.serviceBookingDummy1;
                    ...
                    return InkWell(
                      onTap: () {
                        ...
                        if (statusStr == 'completed' ||
                          ...
                          // Active booking → fetch latest status and resume
                      ...
                      child: Container(
                        ...
                        children: [
                                ...
                                child: Column(
                                  ...
                                  children: [
                                    ...
                                    Column(
                                      ...
                                      children: [
                                        Text(
                                          item['service_name']?.toString() ?? "Service",
                                        ...
                                        if (status == BookingStatus.inProgress)
                                          ...
                                          Builder(builder: (context) {
                                            final double price = double.tryParse(item['total_amount']?.toString() ?? item['price']?.toString() ?? "0") ?? 0;
                                            return Row(
                                              children: [
                                                const Text(
                                                  "Service Fee: ",
                          ...
                          Container(
                            ...
                            child: Column(
                              children: [
                                rowText(UserMessages.addressLabel, item['address']?.toString() ?? "N/A"),
                                SizedBox(height: AppSizes.h(context, 12)),
                                rowText(UserMessages.dateLabel, item['booking_date']?.toString() ?? "N/A"),
                          ...
                          Row(
                            children: [
                              CircleAvatar(
                                ...
                                child: ClipOval(
                                  child: CachedNetworkImage(
                                    imageUrl: (handyman?['profile_image'] != null)
                                        ? handyman!['profile_image'].toString()
  ...
  Future<void> _navigateToActiveBooking(BuildContext context, Map item) async {
    ...
    try {
      ...
      final response = await http.get(url, headers: {
        ...
        if (apiToken != null && apiToken.isNotEmpty) 'Authorization': 'Bearer $apiToken',
      });
      ...
      if (response.statusCode == 200) {
        ...
        data = rawBody is List
            ? (rawBody.isNotEmpty ? Map<String, dynamic>.from(rawBody.first) : {})
        ...
        service = rawSvc is List ? (rawSvc.isNotEmpty ? rawSvc.first : {}) : rawSvc;
      } else {
      ...
      final price = item['total_amount']?.toString() ?? item['price']?.toString();
      final currentStatus = (detail['status'] ?? item['status'] ?? '').toString().trim().toLowerCase();

      debugPrint('📋 BookingResume: ID=$bookingId | Status="$currentStatus" | Handyman=${handyman != null}');

      // ── Route based on live status ──────────────────────────────────────────
      if (currentStatus == 'pending' && handyman == null) {
        // Still searching for a professional
        Navigator.pushNamed(context, AppRoutes.bookingStatus, arguments: {
          'service': service,
        ...
        Navigator.pushNamed(context, AppRoutes.professionalAssigned, arguments: {
          'service': service,
          'booking_data': data,
          'price': price,
          'status': BookingStatusModel(
            ...
            appointmentDate: detail['booking_date'] ?? item['booking_date'] ?? '',
            appointmentTime: detail['booking_slot'] ?? item['booking_slot'] ?? '',
          ),
        });
        ...
        Navigator.pushNamed(context, AppRoutes.serviceVerification, arguments: {
          'service': service,
          'booking_data': data,
          'price': price,
          'status': BookingStatusModel(
        ...
        debugPrint('⚠️ Unknown status "$currentStatus" — defaulting to BookingStatusScreen');
        Navigator.pushNamed(context, AppRoutes.bookingStatus, arguments: {
          'service': service,
          'booking_id': bookingId,
          'date': detail['booking_date'] ?? item['booking_date'],
      ...
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(content: Text('Could not load booking. Please try again.')),
        );
      }
    } finally {
