...
class _BookingHomePageState extends State<BookingHomePage> {
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        if (data != null && mounted) {
          ...
          final bookingDetail = detail is List ? (detail.isNotEmpty ? detail.first : {}) : detail;
          final status = bookingDetail?['status']?.toString().toLowerCase();

          // Auto-navigate if status is in progress or pending approval
          if (status == 'in_progress' || status == 'pending_approval' || status == 'arrived') {
              ...
              child: SingleChildScrollView(
                child: Column(
                  children: [
                    ...
                    PriceDetails(totalAmount: widget.totalAmount),
                  ],
                ),
              ),
            ),
                ...
                child: ElevatedButton(
                  onPressed: _isSubmitting ? null : () async {
                    ...
                    try {
                      ...
                      final double lat = location?['latitude'] != null ? double.parse(location!['latitude'].toString()) : 28.6155;
                      final double lng = location?['longitude'] != null ? double.parse(location!['longitude'].toString()) : 77.2150;
                      final String addressText = location?['address'] ?? "test address";

                      debugPrint("📍 Booking Flow: Using coordinates from selected address");
                      ...
                      if (_selectedDate != null) {
                        ...
                        formattedDisplayDate = "${_selectedDate!.day} ${monthNames[_selectedDate!.month - 1]}, ${_selectedDate!.year}";
                      }
                      ...
                      final url = Uri.parse('${ApiConfig.apiBaseUrl}/booking-save');
                      final requestBody = {
                        "service_id": widget.service is Map ? widget.service['id'] : widget.service.id,
                        "date": dateStr,
                        ...
                        "service_addon_id": widget.selectedAddOns.map((e) => e['id']).toList(),
                        "service_option_id": widget.selectedOption != null ? [widget.selectedOption!['id']] : [],
                      };
                      ...
                      if (response.statusCode == 200) {
                        final data = json.decode(response.body);
                        final bookingId = data['booking_id']?.toString() ?? "#UC876-67";
                        ...
                        if (mounted) {
                          ScaffoldMessenger.of(context).showSnackBar(
                            SnackBar(
                              content: Text("API Error: ${response.statusCode} - ${response.body}"), 
                  ...
                  style: ElevatedButton.styleFrom(
                    backgroundColor: _isSubmitting ? Colors.grey : AppColors.primaryRed,
  ...
  Widget _buildActiveBookingCard() {
    final bData = _activeBooking is List ? (_activeBooking as List).first : _activeBooking;
    final rawDetail = bData?['booking_detail'];
    ...
    final status = (detail?['status'] ?? bData?['status'])?.toString().toUpperCase() ?? "PENDING";
    final providerName = provider?['display_name'] ?? "Handyman";
    final providerImage = service?['provider_image'] ?? provider?['profile_image'];
    ...
    return Container(
      ...
      child: Column(
        children: [
            ...
            children: [
              CircleAvatar(
                ...
                backgroundImage: (providerImage != null && providerImage.toString().startsWith('http'))
                  ...
                  : null,
                child: (providerImage == null) ? const Icon(Icons.person) : null,
              ),
                  ...
                  children: [
                    ...
                    Row(
                      children: [
                        ...
                        Text(
                          (provider?['providers_service_rating'] ?? provider?['handyman_rating'] ?? 0).toString(),
                    ...
                    Text(
                      ...
                      style: TextStyle(
                        color: (status == 'PENDING_APPROVAL' || status == 'IN_PROGRESS' || status == 'COMPLETED')
                        ...
                        child: Consumer<UserProvider>(
                          builder: (context, userProvider, _) {
                            ...
                            return Column(
                              ...
                              children: [
                                Text(
                                  isPaused ? "Paused" : "In Progress: ${formatDuration(elapsed)}",
                                  style: TextStyle(
                                    fontSize: 14,
                                    color: isPaused ? Colors.orange : AppColors.primaryRed,
                                    fontWeight: FontWeight.bold,
                                  ),
                                ),
                                Text(
          ...
          if (status == 'PENDING_APPROVAL' || status == 'IN_PROGRESS' || status == 'COMPLETED')
            Padding(
              padding: const EdgeInsets.only(top: 8),
              child: Row(
                children: [
                    ...
                    child: ElevatedButton(
                      ...
                      child: const Row(
                        ...
                        children: [
                          ...
                          SizedBox(width: 4),
                          Text("Call", style: TextStyle(color: Colors.black, fontSize: 12, fontWeight: FontWeight.bold)),
                        ],
                      ),
                    ),
                  ),
                  ...
                  Expanded(
                    child: ElevatedButton(
                      ...
                      child: const Text("View Progress", style: TextStyle(color: Colors.white, fontSize: 12)),
                    ),
                  ),
                ],
              ),
